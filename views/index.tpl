...
$(
    function() 
    {
        ...
        $('.led_pwm').slider( "disable" );
        
        // Get PWM value, add 1s delay to prevent data to be sent to frequently to the server    
        var pwm_data;
        var sliderChanged = false;
        $('.led_pwm').change(function() {sliderChanged = true;});        
        setInterval(function() 
        {
            if ( sliderChanged ) 
            {
                sliderChanged = false;
                pwm_data = {red: $("#red_pwm").val(), green: $("#green_pwm").val(), blue: $("#blue_pwm").val()};
                $.get('/_pwm', pwm_data);
            }
        }, 1000);
        ...
        $('input:radio[name=radio_method]').click(function() 
        {
            $.get('/_method', {val: $('input:radio[name=radio_method]:checked').val()});
            if($('#radio_toggle').is(':checked'))
            {
                $('.led_pwm').slider( "disable" );
                $('.led_toggle').slider( "enable" );
            }
            if($('#radio_pwm').is(':checked'))
            { 
                $('.led_toggle').slider( "disable" );
                $('.led_pwm').slider( "enable" );
            }
        });             
    }
);
...
<div data-role="page" data-theme="a">
  <div data-role="header">
    <div><h3>GHX ALARM NOTIFICATION</h3></div>
  </div>

  <div data-role="content">
  
<h2>Toggle</h2>
        <label for="switch1">RED:</label>
        <select class="led_toggle" name="switch1" id="red_toggle" data-role="slider" data-mini="true">
            <option value="off">Off</option>
            <option value="on">On</option>
        </select>  
        <label for="switch2">GREEN:</label>
        <select class="led_toggle" name="switch2" id="green_toggle" data-role="slider" data-mini="true">
            <option value="off">Off</option>
            <option value="on">On</option>
        </select>  
        <label for="switch3">BLUE:</label>
        <select class="led_toggle" name="switch3" id="blue_toggle" data-role="slider" data-mini="true">
            <option value="off">Off</option>
            <option value="on">On</option>
        </select>  
